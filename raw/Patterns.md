...
Используются для ограничения нагрузки. Для этого используется *бакет* (ведро) для определенного пользователя с определенной емкостью в токенах `<limit>` (500), с постоянной частотой `<refillInterval>` (0.01s) заполняемый токенами. Каждый запрос извлекает один токен из бакета, а когда бакет оказывается пустым, следующий запрос блокируется.
