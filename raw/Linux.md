...
Если выполнение команды `<команда>` завершится успешно, а именно будет возвращено `0`, то выполнятся комманды от `then` и до `elif` / `else` / `fi` [, иначе выполнятся комманды от `else` и до `fi`].
...
Условие, находящееся внутри квадратных скобок может быть проверено без использования оператора `if`: `if [ <выражение> ]`.
