...
- `CMD <команда1>[ && ...]`.
  ...
  `CMD ["<команда>"[, "<аргумент1_для_команды>"[, ...]]]` - описывает команду с аргументами, которую нужно выполнить когда контейнер будет запущен - команду по умолчанию. Аргументы могут быть переопределены при запуске контейнера. В файле может присутствовать лишь одна инструкция `CMD`. Если в файле есть несколько таких инструкций, система проигнорирует все кроме последней. Аргументы командной строки, передаваемые `docker run`, переопределяют аргументы, предоставленные инструкции `CMD` в **Dockerfile**:
...
- **shell form** - когда запись происходит в виде строки, в этом случае команда выполняется в подоболочке, обычно `/bin/sh -c`. Этот формат позволяет обрабатывать оболочку (например, расширять переменные, использовать подстановочные знаки и т. д.), таким образом делая ее более гибкой для определенных типов команд, но в этом случае процесс, выполняющий команду, не является PID `1` контейнера, что может привести к проблемам с обработкой сигналов, поскольку сигналы, посылаемые Docker:
...
- `docker [image] build [<опции>] <путь_к_каталогу_сборки>` - собирает образ Docker из `Dockerfile` из набора файлов, расположенных по определенному пути `<путь_к_каталогу_сборки>` или URL, с указанными опциями `<опции>`:
    - `--build-arg [<имя_переменной>=<значение>[ ...]` - установливает значения переменных;
    - `-f <путь_к_Dockerfile>` - указывает путь к файлу `Dockerfile`;
...
- `docker image tag <имя_образа> <имя_тега>` - устанавливает тег для образа;
...
- `docker [container] run [<опции>] <образ> [[-it] <команда> [, <аргумент1_для_команды>[ ...]]]` - создает и запускает контейнер на основе указанного образа `<образ>`[, а в нем указаную команду] с указанными опциями `<опции>`:
    - `-p <порт>:<expose_порт>` - с проброшеными портами внутрь;
    ...
    - `--entrypoint=""` - сбрасывает `ENTRYPOINT` в контейнере при запуске;
...
services:
    <имя_сервиса>:
        ...
        healthcheck:
            ...
            test: ["CMD", "<команда>"[, "<аргумент1_для_команды>"[, ...] ]]
            interval: <количество>s
            start_period: <количество>s
...
По умолчанию при запуске переменные окружения берутся из файла `.env`. Для использования их из файла используется подстановка в формате `${<имя_переменной_окружения>}`.
...
`docker compose up [-d] [--no-deps] [--force-recreate] [--build]` - запуск всех контейнеров [в фоне] [не запуская связанные сервисы] [всегда пересоздавая контейнеры] [с выполнением команды `build` для всех сервисов].  
    ...
    template:
      ...
      spec:
        ...
        - name: <имя_приложения>
          ...
          ports:
          - containerPort: <порт_приложения>
```

Создание сервиса для микросервиса:
```yaml
apiVersion: v1
kind: Service
metadata:
...
spec:
  ...
  ports:
    - protocol: TCP
      port: <внешний_порт>
      targetPort: <внутренний_порт>
  type: LoadBalancer
```

Для доступа используется внутри DNS-имя `<имя_приложения>.default.svc.cluster.local`.
...
**Релиз** (Release) - это развёрнутые инстансы чартов в кластере Kubernetes. Каждый раз, когда чарт устанавливается в кластер, создаётся новый релиз, который можно обновлять, отменять и удалять - эффективно управлять жизненным циклом приложений.

Релизы позволяют управлять состоянием развёрнутых приложений, обеспечивают контроль версий и возможность отката, что особенно полезно для управления обновлениями и восстановлением в случае проблем.
...
- при использовании `[for..in]` структурой будет список и `<выражение>` должно быть значением:
    > `[for i, v in var.list : "${i} is ${v}"]`
- при использовании `{for..in}` структурой будет карта и `<выражение>` должно быть: `<ключ> => <значение>`:
...
- `-target="<имя_ресурса>"` - указывает на то, какой ресурс необходимо применить.
