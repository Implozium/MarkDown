...
Для изменения хранилища используются мутации: `this.$store.commit('<мутация>'[, <арг1>[, ...]])` или:
```javascript
this.$store.commit({
    type: '<мутация>'
    [, <свойство1>: <значение>
...
Действия поддерживают тот же формат что и мутации для передачи нагрузки, а также объектный синтаксис. Вызываются: `this.$store.dispatch('<действие>'[, <арг1>[, ...]])`. При этом `dispatch` возвращает то что возвращает само действие.
