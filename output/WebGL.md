...
<a id="Инициализация-canvas" href="#Инициализация-canvas">Инициализация, `canvas`</a>
=======================
...
<a id="Система-координат" href="#Система-координат">Система координат</a>
=================
...
<a id="Шейдер" href="#Шейдер">Шейдер</a>
======
...
`gl.getProgramParameter(<шейдерная_программа>, gl.LINK_STATUS);` - возвращает `true` если шейдерная программа успешно скомпилированна, иначе `false`.
...
`gl.drawArrays({gl.POINTS | gl.LINES | gl.LINE_STRIP | gl.LINE_LOOP | gl.TRIANGLES | gl.TRIANGLE_STRIP | gl.TRIANGLE_FAN}, <номер_первой_вершины>, <количество_вершин>);` - выполняет вершинный шейдер `<количество_вершин>` раз, чтобы нарисовать фигуры, определяемые первым параметром.

## <a id="Код-шейдеров" href="#Код-шейдеров">Код шейдеров</a>
...
### <a id="attribute-переменные" href="#attribute-переменные">`attribute` переменные</a>
...
### <a id="uniform-переменные" href="#uniform-переменные">`uniform` переменные</a>
...
`gl.uniform{1 | 2 | 3 | 4}f(<ссылка_на_переменную_юниформ>, <значение0>[, <значение1>[, <значение2>[, <значение3>]]]);` - задание значения uniform-переменной, все по анологии с переменной атрибут.
...
### <a id="Вершинный-шейдер" href="#Вершинный-шейдер">Вершинный шейдер</a>

Глобальные переменные:
- `vec4 gl_Position` - определяет координаты вершины;
- `float gl_PointSize` - определяет размеры точки в пикселях;

### <a id="Фрагментный-шейдер" href="#Фрагментный-шейдер">Фрагментный шейдер</a>
...
<a id="Буферы" href="#Буферы">Буферы</a>
======
...
`gl.vertexAttribPointer(<ссылка_на_переменную_атрибут>, {1 | 2 | 3 | 4}, {gl.UNSIGNED_BYTE | gl.SHORT | gl.UNSIGNED_SHORT | gl.INT | gl.UNSIGNED_INT | gl.FLOAT}, {false | true}, <количество_байт_разделения_между_элементами>, <смещение_в_массиве>);` - присваивает буферный объект переменной отрибуту, где:
...
`gl.enableVertexAttribArray(<ссылка_на_переменную_атрибут>);` - включает разрешение присваивания переменной-атрибут.  
`gl.disableVertexAttribArray(<ссылка_на_переменную_атрибут>);` - включает разрешение присваивания переменной-атрибут.