...
- [Тестирование](#user-content-Тестирование)
    ...
    - [Пример тестирования в браузере](#user-content-Пример-тестирования-в-браузере)
- [Effector](#user-content-Effector)
    - [React](#user-content-React)
- [ReactQuery](#user-content-ReactQuery)
...
## <a id="Пример-тестирования-в-браузере" href="#Пример-тестирования-в-браузере">Пример тестирования в браузере</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
<a id="Effector" href="#Effector">Effector</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
========

Библиотека `effector`.

`const <event> = createEvent()` - создает событие.  
`<event>.watch((<данные>) => {/* наблюдение за событием */})` - регистрирует обработчик, который будет вызываться при вызове события.

`<event>([<данные>])` - вызывает событие [с определенными данными].

`const <effect>Fx = createEffect(([<данные>]) => {/* асинхронная функция */})` - создает эффект. Эффект обладает:
- `<effect>Fx.doneData` - событие успешного завершения в котором содержатся данные;
- `<effect>Fx.failData` - событие неуспешного завершения в котором содержатся ошибки;
- `<effect>Fx.pending` - флаг того, что функция выполняется.

`<effect>Fx([<данные>])` - вызывает эффект [с определенными данными].

`const $<store> = createStore(<изначальное_значение>)` - создает хранилище.  
`$<store>.on(<event>, (<store>[, <данные>]) => {/* изменение хранилища */; return <новое_значение>})` - регистрирует обработчик события по которому будет изменяться хранилище, где `store` - это текущее значение в хранилище и обработчик должен возвращать новое значение для хранилища.  
`$<store>.watch((<store>) => {/* наблюдение за хранилищем */})` - регистрирует обработчик, который будет вызываться при изменении хранилища.

Связывает хранилище и эффект, который запустится когда функция фильтра вернет true.
```ts
sample({
    source: $<store>,
    filter: (<store>) => {/* предикат фильтрации */; return <result>},
    target: saveCountFx,
})
```

`combine($<store_1>[, ...], (<store_1>[, ...]) => {/* комбинация хранилищ*/; return <новое_хранилище>;})`

## <a id="React" href="#React">React</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

`const <значение> = useStore($<store>)` - возвращает значение из хранилища.

<a id="ReactQuery" href="#ReactQuery">ReactQuery</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
==========
...
Хук `useQuery` используется для создания запроса:
`const <result> = useQuery({ queryKey: <ключ>, queryFn: <функция_запроса>}[, <опции>]);` - создает запрос, где:
- `<ключ>` - идентификатор запроса, может быть массивом значений;
- `<функция_запроса>` - асинхронная функция, которая должна возвращать Promise;
- `<опции>` - объект опций для запроса, который содержит свойства:
    - `active` - булевый флаг о том, что запрос нужно выполнить;
- `<result>` - объект результата, который содержит свойства:
    - `data` - значение, которым разрешился Promise из функции `<функция_запроса>`;
    - `error` - ошибка, которым отвергся Promise из функции `<функция_запроса>`;
    - `isError` - флаг того, что функция `<функция_запроса>` завершилась ошибочно;
    - `isSuccess` - флаг того, что функция `<функция_запроса>` завершилась успешно;
    - `isPending` - флаг того, что функция `<функция_запроса>` еще не выполнялась из-за опции `active`;
    - `isFetching` - флаг того, что функция `<функция_запроса>` запрашивает данные - Promise еще не разрешился;
    - `refetch` - метод, чтобы `<функция_запроса>` перезапрасила данные.

Хук `useMutation` используется для изменения данных:
`const <mutation> = useMutation({mutationFn: <функция_мутации>}[, <опции>]);` - создает мутацию, где:
- `<функция_мутации>` - асинхронная функция, которая должна возвращать Promise;
- `<mutation>` - объект который содержит:
    - `mutate` - метод, который вызывает `<функция_мутации>`, в него нужно передавать все аргументы как в функцию: `<функция_мутации>`, вторым аргументом принимает объект опций аналогичный `<опции>`;
    - `mutateAsync` - метод аналогичный методу `mutate`, только возвращает Promise, который разрешится или отвергнется результатом функции `<функция_мутации>`;
    - `data` - значение, которым разрешился Promise из функции `<функция_мутации>`;
    - `error` - ошибка, которым отвергся Promise из функции `<функция_мутации>`;
    - `isPending` - флаг того, что функция `<функция_мутации>` еще выполняется;
    - `isError` - флаг того, что функция `<функция_мутации>` завершилась ошибочно;
    - `isSuccess` - флаг того, что функция `<функция_мутации>` завершилась успешно;
    - `isIdle` - флаг того, что функция `<функция_мутации>` еще не выполнялась;
    - `reset` - метод, который сбрасывает все данные в мутации;
- `<опции>` - объект опций для запроса, который содержит хуки вызова:
    - `onMutate: (variables) => {/* <контекст> */}` - метод, который формирует контекст для вызова;
    - `onError: (error, variables, onMutateResult, context) => {/* <код> */}` - метод, который вызовится в случае ошибки;
    - `onSuccess: (data, variables, onMutateResult, context) => {/* <код> */}` - метод, который вызовится в случае успеха;
    - `onSettled: (data, error, variables, onMutateResult, context) => {/* <код> */}` - метод, который вызовится в любом случае;
    - `retry` - количество попыток для мутаций в случаи ошибки (по умолчанию 1).

`const <queryClient> = useQueryClient();` - возвращает клиента - объект который содержит:
- `invalidateQueries([<ключ>[, {exact: true}]])` - сбрасывает все запросы [только по префиксу ключу `<ключ>` [по полному совпадению]];
- `setQueryData(<ключ>, <данные>)` - устанавливает данные `<данные>` по ключу `<ключ>`.