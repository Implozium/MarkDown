...
`navigator.serviceWorker.register('<путь_к_js_файлу>');` - регистрирует сервис воркера, если он не был уже зарегистрирован и возвращает `Promise` для сообщения о результате регистрации. Сервис воркер будет получать события `fetch` в рамках своего пути подключения, т.е. воркер по пути `<путь>/<js_файл>` будет получать только все события `<путь>/` и не выше.
