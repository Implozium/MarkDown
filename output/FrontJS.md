...
- [Документ, события, интерфейсы](#Документ-события-интерфейсы)
    ...
    - [Размеры и прокрутка страницы](#Размеры-и-прокрутка-страницы)
        ...
        - [Видимость страницы](#Видимость-страницы)
...
- [Основы работы с событиями](#Основы-работы-с-событиями)
    ...
    - [Обработка событий](#Обработка-событий)
        ...
        - [Основные свойства и методы объекта события `Event`](#Основные-свойства-и-методы-объекта-события-Event)
        - [Всплытие](#Всплытие)
...
- [Формы, элементы управления](#Формы-элементы-управления)
    - [Навигация и свойства элементов формы](#Навигация-и-свойства-элементов-формы)
        - [Проверки ошибок](#Проверки-ошибок)
    - [Файлы](#Файлы)
...
`<abortController>.abort();` - вызывает прерывание в сигнале - то есть генерирует событие `abort` и сигнал вызывает прерывание там где он передан.  
...
`<input>.checkValidity()` - возвращает `true`, если на элементе есть ошибки данных, а иначе `false` и отправляет событие `invalid` на элемент со свойством `cancelable = true`.  
`<input>.reportValidity()` - возвращает `true`, если на элементе есть ошибки данных, а иначе `false` и отправляет событие `invalid` на элемент со свойством `cancelable = true`, а также сообщает вне элемента что есть ошибка.  
...
Методы:
- `<dialog>.show()` / `<dialog>.showModal()` показывают диалог модального вида;
...
class <CustomElement> extends HTMLElement {
    ...
    /**
     * вызывается при изменении атрибута за которым следят
    ...
    formResetCallback() {}
    /**
     * вызывается когда `disabled` изменяется у компонента или fieldset в котором он находится
...
`customElements.get('<custom-element>');` - возвращает зарегистрированый элемент, его класс, если он есть.
...
Методы слота:
- `<slot>.assignedNodes([{ flatten: true }])` - который возвращает массив узлов находящихся в этом слоте [в виде одномерного массива];
- `<slot>.assignedElements([{ flatten: true }])` - который возвращает массив элементов находящихся в этом слоте [в виде одномерного массива].

События слота:
- `slotchange` - вызывается, когда к слоту присоединяется или отсоединяется связанный со слотом элемент.

У элемента связанного со слотом есть свойство `assignedSlot` - которое является ссылочным типом на слот, если он есть и при `mode` равном `'open'` или `null`.

Для доступа к элементам слота можна использовать селектор: `::slotted(<селектор>)`.
...
`<internals>.setFormValue(<value>)` - устанавливает значение формы (если оно равно `null` то значение участвует в отправке формы) по имени равному значению атрибуты `name` веб компонента.
...
`<blob>.stream()` - возвращает `ReadableStream`, с помощью которого можно прочитать `Blob` объект.
...
- `<controller>.write(<chunk>)` - записывает данные в поток `<chunk>` и возвращает промис, который разрешится после успешной записи, или отвергнется если была ошибка в потоке;
- `<controller>.releaseLock()` - освобождает поток.
