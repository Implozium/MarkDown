...
- [Архитектура](#user-content-Архитектура)
    - [FSD (Feature-Sliced Design).](#user-content-FSD-Feature-Sliced-Design)
...
- массива из объектов со свойствами:
    - `<css-свойство-1>` - css свойство и его значение (свойство `offset` должно быть `cssOffset`);
...
- объект со свойствами и значениями в виде массивов для описания кадров:
    - `<css-свойство-1>` - css свойство и его значение массив значений для анимации(свойство `offset` должно быть `cssOffset`);
...
- `auxclick` - возникает, когда кликнули на элемент не левой кнопкой мыши;
...
`monitorEvents(<HTML_элемент>, { '<событие>' | ['<событие_1>'[, ...]] });` - устанавливает мониторинг событий элемента, при каждом вызове события выводит в консоль сведения об этих событиях.  
`unmonitorEvents(<HTML_элемент>, { '<событие>' | ['<событие_1>'[, ...]] });` - убирает мониторинг событий элемента.
...
class <CustomElement> extends HTMLElement {
    ...
    constructor() {
        super();
        const shadowRoot = this.attachShadow({ mode: 'open' | 'close', delegateFocus: true }); // возвращает (и добавляет при 'open') ShadowRoot для сокрытия содержимого элемента, где 'open' обозначает, что в свойстве `shadowRoot` будет содержаться ShadowRoot, а 'close' - что там его не будет; при delegateFocus = true будет прокидываться фокус внутрь
     ...
     * до того как будет считано его содержимое
...
Атрибут `part` у элементов со значением `<part_1>[ ...]` позволяет прокидывать элементы за shadow-tree, что позволяет их использовать в селекторе `::part(<part_1>)` вне shadow-tree.
> `<span part="title main">Title</span>`  
> `custom-element::part(title) {...}`

Атрибут `exportparts` со значением `<part_1>[: <алиас_1>][ ...]` на самом веб компоненте позволяет прокидывать внутреннии `part` на уровень выше.
...
`URL.createObjectURL(<объект>)` - создает из объекта `<объект>` типа `File` или `Blob` строку вида `blob:<сайт>/<uuid>` в которой объект будет закодирован и его можно будет передать по ссылке в нужное место;  
`URL.revokeObjectURL(<строка>)` - освобождает объект, который был закодирован по ссылке.
...
`let <blob> = new Blob([[<элемент_1>[, ...]]][, {type: '<MIME_тип>'}]);` - создает `Blob` объект с элементами [и определенным MIME типом].
...
- `cache.addAll([<url_1>[, ...]])` - возвращает `Promise`, который завершается успешно когда все указанные ссылки из масссива будут добавлены в кэш;
...
`const <time> = performance.now();` - возвращает высокоточную отметку времени в виде числа.

<a id="Архитектура" href="#Архитектура">Архитектура</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
===========

## <a id="FSD-Feature-Sliced-Design" href="#FSD-Feature-Sliced-Design">FSD (Feature-Sliced Design).</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

**Слой** - это базовый каталог. Модули (файлы) на одном слое могут знать только о модулях со слоев строго ниже, и как следствие, импортировать только с них.

Чем выше слой, тем больше он завязан на предметную область и тем шире его знания о приложении (может использовать "нижележащие" слои). Чем ниже слой, тем больше его влияние на систему и тем опаснее вносить в него изменения.

Модули на одном уровне, не могут использовать модули с этого уровня кроме слоев `app`, `shared`. Модули не могут вкладываться в другие модули и должны быть плоским списком внутри слоя.

Слои (в приоритете от высшего к низшему):
1. `app` - может использовать слои: `app`, `pages`, `widgets`, `features`,  `entities`, `shared`. Отвечает за:
    - точку входа в приложение;
    - определяет роутинг;
    - инициализирует локаль;
    - настраивает глобальные объекты (RUM, error-counter, метрика);
    - подключает глобальные стили;
2. `pages` - может использовать слои: `widgets`, `features`,  `entities`, `shared`. Отвечает за:
    - страницы приложения, которые должны иметь максимально простую логику;
    - работу с query-параметрами, без взаимодействия с path, который определяется на уровне слоя app;
3. `widgets` - может использовать слои: `features`,  `entities`, `shared` Отвечает за:
    - смысловые блоки, которые представляют самодостаточную композицию модулей с понятной область ответственности и состоят из композиции слоев `features`, `entities`, `shared`;
    - должны не зависеть от логики конкретных страниц;
4. `features` - может использовать слои: `entities`, `shared`. Отвечает за:
    - функциональные блоки бизнес логики, которые можно сформулировать как действие с бизнес-сущностями;
    - определяет вопросами: что пользователь может делать в приложении? и это будет напрямую использовать конечный пользователь?
5. `entities` - может использовать слои: `shared`. Отвечает за:
    - бизнес-сущности, с которыми работает пользователь и вокруг которых построена функциональность приложения:
        - логику получения / сохранения данных;
        - хранилище для для хранения и предоставления данных;
        - базовые UI-компоненты сущностей, для построения вышележащих слоев;
        - не должна содержать:
            - логику для получения данных для конкретной фичи;
            - UI-компоненты, предназначенные для её редактирования (Input, Checkbox, Form и т.п.);
            - UI-компоненты, указывающие на окружение в котором компонент должен быть использован (Modal, Drawer и т.п.);
6. `shared` - может использовать слои: `shared`. Отвечает за:
    - базовые ui-компоненты (кнопка, попап, ...);
    - клиенты для работы с api (только базовая логика работы с запросами, без знаний конкретных методов api);
    - библиотеки общего назначения (работа с датой и временем, работа с DOM, работа с валютой, ...);
    - не должен содержать бизнес-логики;

Структура слоев:
- `<слой>`:
    - `<модуль>`:
        - `<сегмент>` - может быть специфичным для модуля;
        - `components` (`ui`) - может использовать слои: `components`, `model`, `api`, `lib`. Содержит компоненты, реализующие UI-логику модуля, которые отвечают только за представление.
        - `model` - может использовать слои: `model`, `api`, `lib`. Содержит код для работы с бизнес-логикой (логика работы со стором, так и другие операции, операции с данными формы, валидация сохраняемых данных и т.п.);
        - `api` - может использовать слои: `api`, `lib`. Содержит код для работы с сетевыми запросами к API (тексты запросов GraphQL, инстанс клиента для запросов, мапперы данных);
        - `lib` - может использовать слои: `lib`. Содержит утилитарный код специфичный для модуля;
        - `index.ts` - точка входа в модуль.

**Сегмент** - это составные части модуля.