...
Функция по умолчанию возвращает кортеж нулевого размера и его написание можно опустить.
...
Вместо замыканий, в качестве значений можно использовать функции.
...
`let <content>: Result<String> = fs::read_to_string(<имя_файла>);` - возвращает текстовое содержимое файла в виде `Result<String>` из модуля `std::fs`.
