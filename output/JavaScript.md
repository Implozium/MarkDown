...
- `Array.from(<псевдомассив>[, function (item, i, arr) {...}[, <this_контекст>]])` - (ES6) возвращает настоящий массив на основе псевдомассива или итериреумых объектов (у которых есть метод `[Symbol.iterator]`)[ и к каждому элементу применяет функцию и результат этой функции заносит в массив]. Псевдомассив может быть такого вида: `{length: <количество>}`;
...
`new <FunctionConsructor>([<арг1>[, ...]])` - создание объекта через функцию конструктор.  
`new <FunctionConsructor>` - аналогично предыдущему варианту, с пустым конструктором.
...
`Object.defineProperties(<объект>, {<свойство1>: <дескриптор>[, ...]})` - позволяет объявить несколько свойств сразу, возвращает переданый `<объект>`.  
...
`Object.seal(<объект>)` - запрещает добавление и удаление свойств, все текущие свойства делает configurable: `false`, возвращает переданый `<объект>`.
`Object.freeze(<объект>)` - запрещает добавление, удаление и изменение свойств, все текущие свойства делает `configurable`: `false`, `writable`: `false`, возвращает переданый `<объект>`.  
...
- промис:
    - `Promise.<<тип_разрешаемого_значения>>` - где `<тип_разрешаемого_значения>` означает тип, которым разрашиться промис 
...
- `.skip`:
    ...
    - после `describe` позволяет пропустить группу тестов, лучше использовать вместо коментирования самих тестов;
...
- `.only`:
    ...
    > `describe.only("<имя_тестируемой_функции>", function () {...})`
...
Глобальные тесты должны распологаться по путь `tests/` в корне проекта.
...
`<spy>.calledWith([<аргумент1>[, ...]])` - возвращает `true` если функция была вызвана с указаными аргументами, иначе `false`.  
...
`<stub>.withArgs([<аргумент1>[, ...]]).<метод_возврата_значения>` - `<метод_возврата_значения>` (к примеру как `returns` и `throws`) будут вызваны только при вызове функции с указаными аргументами, а иначе будут вызваны другие `<метод_возврата_значения>`.  
...
`<stub>.callsArg(<номер>)` - вызывает указаный переданый аргумент под номером `<номер>` как функцию при вызове заглушки.  
`<stub>.callsArgWith(<номер>[<аргумент1>[, ...]])` - вызывает указаный переданый аргумент под номером `<номер>` как функцию при вызове заглушки с указаными аргументами.  
`<stub>.restore()` - востанавливает исходную функцию.
...
`const <mock> = sinon.mock(<объект>);` - возвращает имитацию для указанного объекта, с предварительно запрограммированным поведением (как заглушка), а также предварительно запрограммированным ожиданием, сам объект не изменяется, но обертывает mock-обект для установки ожиданий на методах объекта.

`<mock>.restore()` - востанавливает все методы имитации.  
