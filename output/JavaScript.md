...
- `new Date(<год_из_4_цифр>, <месяц_с_0>[, <день>, <час>, <мин>, <сек>, <мс>])`.
...
`<stub>.withArgs([<аргумент1>[, ...]]).<метод_возврата_значения>` - `<метод_возврата_значения>` (к примеру как `returns` и `throws`) будут вызваны только при вызове функции с указанными аргументами, а иначе будут вызваны другие `<метод_возврата_значения>`.  
