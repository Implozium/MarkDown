...
- [Тестирование](#user-content-Тестирование)
    - [Автоматические тесты при помощи chai и mocha](#user-content-Автоматические-тесты-при-помощи-chai-и-mocha)
        ...
        - [Виды `chai.expect`](#user-content-Виды-chaiexpect)
        - [Асинхронные тесты](#user-content-Асинхронные-тесты)
        - [Размещение тестов](#user-content-Размещение-тестов)
        - [Sinon](#user-content-Sinon)
            - [Шпион `spy`](#user-content-Шпион-spy)
            - [Заглушки `stub`](#user-content-Заглушки-stub)
            - [Имитация `mock`](#user-content-Имитация-mock)
    - [Автоматические тесты при помощи jest](#user-content-Автоматические-тесты-при-помощи-jest)
        - [Имитация `mock`](#user-content-Имитация-mock)
        - [Проверки](#user-content-Проверки)
            - [Проверки значений](#user-content-Проверки-значений)
            - [Проверки вызова функций](#user-content-Проверки-вызова-функций)
    - [Цикломатическая сложность](#user-content-Цикломатическая-сложность)
    - [Покрытие кода](#user-content-Покрытие-кода)
    - [Приемы тестирования](#user-content-Приемы-тестирования)
...
`let [{<переменная1>[ = <значение_по_ум>] | }[, ...][, ...<переменная_все_остальное>]] = [<значение>[, ...]];` - при таком присвоении первое значение массива пойдет в переменную `<переменная1>`, второе - в следующую, а последующие (если есть) - будут отброшены. Ненужные элементы массива также можно отбросить, поставив лишнюю запятую.
...
`const <performanceMeasure> = performance.measure('<имя_измерителя>', '<имя_метки1>', '<имя_метки2>')` - создает интервал `PerformanceMeasure` между метками `<имя_метки2>` и `<имя_метки1>`. Содержит свойства:
...
- `Array.of(<значение1>[, ...])` - возвращает массив на основании переданных значений, аналогично `new Array(<значение1>[, ...])`, но без специального поведения при одном аргументе.
...
`<массив>.toSpliced(<номер_индекс>[, <кол_элементов>[, <элемент1>[, <элемент2>[, ...]]]])` - аналогично методу `toSpliced`, но не изменяет текущий массив, а возвращает новый.
...
`<массив>.concat(<элемент1>[, <элемент2>[, ...]])` - создает новый массив и возвращает его, в который копируются элементы из массива, а также указанные элементы. Если аргумент `concat` - массив, то `concat` добавляет элементы из него.
...
Должен реализовываться как свойство которое содержит: `{ <свойство1>: true[, ...] }` свойства, которые можно использовать внутри `with`.
...
```javascript
[async] function* <функция_генератор>([<параметр1>[, ...]]) {
    [let <переменная> = ]yield <возвращаемое_значение1>;
...
Основным методом генератора является `<генератор>.next()`. При вызове он возобновляет выполнение кода до ближайшего ключевого слова `yield`. По достижении `yield` выполнение приостанавливается и возвращается значение `{value: <возвращаемое_значение1>, done: false}` во внешний код.
...
**Асинхронные генераторы** выглядят как обычные генераторы, только которые должны возвращать через `yield` `Promise`, который должен разрешаться объектом `{value: <возвращаемое_значение1>, done: false}` и для return` - `{value: <возвращаемое_значение>, done: true}`. В случае использования `async`, будет возвращаться автоматически `Promise`.
...
`Object.assign(<целевой_объект>, <объект1>[, ...])` - получает список объектов и копирует в первый `<целевой_объект>` свойства из остальных. При этом последующие свойства перезаписывают предыдущие.  
`Object.is(<значение1>, <значение2>)` - возвращает `true`, если значения `<значение1>` и `<значение2>` равны, иначе `false`.
...
`Object.fromEntries(<итерируемый_объект>)` - создает и возвращает объект, инициализированный списком значений, должен быть итерируемый объект (не обязательно именно массив), например: `[[<ключ1>, <значение>][, [<ключ2>, <значение>][, ...]]]`.
...
`<функция>.call(<this_контекст>[, <аргумент_функции1>[, ...]])` - вызывается функция, при этом первый аргумент `call` `<this_контекст>` становится ее `this`, а остальные передаются "как есть" для функции.  
...
`<функция>.bind(<this_контекст>[, <аргумент_функции1>[, ...]])` - вернет функцию-обертку, которая фиксирует контекст и передает вызовы в `<функция>`, при этом если указаны аргументы - они будут прибавлены к каждому вызову новой функции, причем встанут перед теми, которые указаны при вызове.
...
`var <timerId> = setTimeout(<функция>, <задержка_мс>[, <аргумент_для_функции1>[, ...]])` - исполнение `<функция>` произойдет спустя время, указанное в `<задержка_мс>`, возвращает числовой идентификатор таймера `<timerId>`, который можно использовать для отмены действия.

`clearTimeout(<timerId>)` - отменяет исполнение таймера.

`var <timer> = setInterval(<функция>, <задержка_мс>[, <аргумент_для_функции1>[, ...]])` - исполнение `<функция>` произойдет спустя время, указанное в `<задержка_мс>`, а затем будет повторятся через каждые `<задержка_мс>`, возвращает числовой идентификатор таймера `<timerId>`, который можно использовать для отмены действия.
...
class <Класс>[extends <Родитель>] {
    [static] [accessor] <публичное_поле1>[ = <значение>]; // создание поля с публичным модификатором доступа, т.е. есть возможность изменять эту переменную вне методов класса
    // ...
    [static] #<приватное_поле1>[ = <значение>]; // создание поля с приватным модификатором доступа, т.е. нет возможности изменять эту переменную вне методов класса
    // ...
...
## <a id="Автоматические-тесты-при-помощи-chai-и-mocha" href="#Автоматические-тесты-при-помощи-chai-и-mocha">Автоматические тесты при помощи chai и mocha</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
### <a id="Дополнительные-методы-тестирования" href="#Дополнительные-методы-тестирования">Дополнительные методы тестирования</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
### <a id="Виды-chaiassert" href="#Виды-chaiassert">Виды `chai.assert`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
### <a id="Виды-chaiexpect" href="#Виды-chaiexpect">Виды `chai.expect`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
### <a id="Асинхронные-тесты" href="#Асинхронные-тесты">Асинхронные тесты</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
### <a id="Размещение-тестов" href="#Размещение-тестов">Размещение тестов</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
### <a id="Sinon" href="#Sinon">Sinon</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

#### <a id="Шпион-spy" href="#Шпион-spy">Шпион `spy`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
#### <a id="Заглушки-stub" href="#Заглушки-stub">Заглушки `stub`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
#### <a id="Имитация-mock" href="#Имитация-mock">Имитация `mock`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
## <a id="Автоматические-тесты-при-помощи-jest" href="#Автоматические-тесты-при-помощи-jest">Автоматические тесты при помощи jest</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
### <a id="Проверки" href="#Проверки">Проверки</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

#### <a id="Проверки-значений" href="#Проверки-значений">Проверки значений</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
#### <a id="Проверки-вызова-функций" href="#Проверки-вызова-функций">Проверки вызова функций</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
## <a id="Цикломатическая-сложность" href="#Цикломатическая-сложность">Цикломатическая сложность</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
## <a id="Покрытие-кода" href="#Покрытие-кода">Покрытие кода</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
## <a id="Приемы-тестирования" href="#Приемы-тестирования">Приемы тестирования</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
