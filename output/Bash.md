...
`groups` - выводит список групп, в которых состоит текущий пользователь.  
...
`chmod [[u][g][o][a]{+ | - | =}[w][r][x]] <файл>` - устанавливает права на файл или каталог, где: `u` - для пользователя, `g` - группы, `o` - других и `a` - для всех.  
...
`chown <имя_пользователя> <файл>` - устанавливает владельца на файл или каталог.
> `chown luser public`

`chgrp <имя_группы> <файл>` - устанавливает группу владельцев на файл или каталог.
> `chgrp geekpress public`

<a id="Другие-команды" href="#Другие-команды">Другие команды</a>
==============

`alias <алиас>="<список_команд>"` - задает алиас для спика команд. Используется как `<алиас>`.
