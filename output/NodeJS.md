...
- [Node.JS](#user-content-NodeJS)
    ...
    - [Стандартные модули](#user-content-Стандартные-модули)
        ...
        - [Модуль `test`](#user-content-Модуль-test)
            - [Моки `mock`](#user-content-Моки-mock)
        - [Модуль `async_hooks`](#user-content-Модуль-asynchooks)
...
- `--max-old-space-size=<количество_операвтивной_памяти_в_мб>` - устанавливает максимальное количество оперативной памяти для приложения;
...
`NODE_OPTIONS=<опции>` - переменная окружения, которая устанавливает опции запуска.
...
`<eventEmitter>.emit("<событие>"[, <аргумент_1>[, ...]])` - вызывает всех слушателей события по порядку подписки, с указанными аргументами.
...
`Buffer.byteLength(<строка>[, <кодировка>])` - возвращает длину строки в байтах [в заданой кодировке `<кодировка>`]
...
```json
{
    ...
    "files": [
        "<файл_или_каталог_включения_к_пакету_1>"
        ],
    "repositories": {
    ...
    "dependencies": {
        "<имя_пакета_1>": "[<|>][=]<номер_версии>[ [<|>][=]<номер_версии>]|*"
    },
    "devDependencies": {
        "<имя_пакета_1>": "[<|>][=]<номер_версии>[ [<|>][=]<номер_версии>]|*"
    },
    ...
    "overrides": {
        "<имя_пакета_1>": {
            "<имя_зависимости_пакета_1>": "$<имя_общей_зависимости_1>",
        }
    },
    "scripts": {
    ...
    "imports": {
        "#<имя_алиаса_1>": "<шаблон_или_путь_к_файлу_или_каталогу>",
        "#<имя_алиаса_2>": [
            "<шаблон_или_путь_к_файлу_или_каталогу_1>"
        ],
        "#<имя_алиаса_3>": {
            "<тип_установки_1>": "<шаблон_или_путь_к_файлу_или_каталогу>",
...
`npx [node@<номер_версии>] <имя_модуля>[@<номер_версии>][ <аргумент_1>[ ...]]` - запускает [используя указанную версию node] модуль как команду, что приводит к автоматическому поиску нужного файла запуска в папке проекта `node_modules`.
...
### <a id="npmrc" href="#npmrc">`.npmrc`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

Это файл с опциями для npm.

Опции:
- `min-release-age=<количество_дней>` - указывает минимальное со времени релиза пакета;
...
`util.inherits(<объект_1>, <объект_2>)` - производит наследование `<объект_1>` от `<объект_2>`, эквивалентно записи `<объект_1>.prototype = new <объект_2>();`.
...
- `response` - объект ответа:
    ...
    `response.flushHeaders();` - отправляет заголовки, без ожидания окончания формирования всего ответа и его завершения, путем вызова `response.end()`.
...
`path.join('<путь_1>'[, ...])` - соединяет пути и нормализует и возвращает его.  
`path.parse('<путь>')` - возвращает объект пути строки с `root`, `dir`, `base`, `ext`, `name`.  
`path.resolve(['<путь_1>'[, ...])` - разрешает последовательность путей или части путей в абсолютный путь, если много путей с абсолютным путем, то выберется последний и возвращает его.  
...
`path.sep` - содержит разделитель, который используется в системе.  
...
describe("<описание_тестового_кейса>", () => {
    test("<имя_теста_1>", [async] (context) => {
        // тело теста
    });
    // ...
});
...
Где `context` определяет контекст выполнения теста и позволяет:
- переопределять поведение таймера: `context.mock.timers.enable({apis: ['setTimeout']});` чтобы с помощью `context.mock.timers.tick(<мс>);` проходить время таймера.

```js
test('mock setTimeout', async (context) => {
    contextt.mock.timers.enable({ apis: ['setTimeout'] });
...
#### <a id="Моки-mock" href="#Моки-mock">Моки `mock`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

Используется для мокирования.

`import {mock} from 'node:test;`

`const <mock_function> = mock.fn([<функция_оригинал>[, <функция_реализация>]][, {times: <количество_вызовов_функции_реализация>}]);` - создает моковую функцию [которая оборачивает функцию `<функция_оригинал>` и проксируется к ней все вызовы [и при вызове использует `<функция_реализация>`]] [и вызывает `<функция_реализация>` вместо оригинала указанное количество раз `<количество_вызовов_функции_реализация>`].

`<mock_function>.mock.callCount();` - возвращает количество вызовов моковой функции.

### <a id="Модуль-asynchooks" href="#Модуль-asynchooks">Модуль `async_hooks`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

Используется для создания общего хранилища (контекста) для вызова асинхронных запросов.

`import { AsyncLocalStorage } from 'node:async_hooks';`

`сonst <async_local_storage> = new AsyncLocalStorage();` - создает асинхронное хранилище.  
`<async_local_storage>.run(<хранилище>, <функция>);` - выполняет функцию `<функция>` с асинхронным хранилищем `<хранилище>`.  
`const <storage> = <async_local_storage>.getStore();` - возвращает значение хранилища, которое можно использовать внутри функции, которая была передана в метод `run`.
