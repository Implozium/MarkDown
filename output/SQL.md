...
- [Основные понятия](#user-content-Основные-понятия)
    - [Реляционные базы данных](#user-content-Реляционные-базы-данных)
    - [Реляционные связи между таблицами баз данных](#user-content-Реляционные-связи-между-таблицами-баз-данных)
- [Типы команд SQL](#user-content-Типы-команд-SQL)
    - [Запись SQL-операторов](#user-content-Запись-SQL-операторов)
    - [Типы данных языка SQL, определенные стандартом](#user-content-Типы-данных-языка-SQL-определенные-стандартом)
        - [Символьные данные](#user-content-Символьные-данные)
        - [Битовые данные](#user-content-Битовые-данные)
        - [Точные числа](#user-content-Точные-числа)
        - [Округленные числа](#user-content-Округленные-числа)
        - [Дата и время](#user-content-Дата-и-время)
        - [Интервал](#user-content-Интервал)
        - [Домен](#user-content-Домен)
        - [Преобразование типов](#user-content-Преобразование-типов)
    - [Выражения](#user-content-Выражения)
        - [Переменные](#user-content-Переменные)
- [Управляющие конструкции SQL](#user-content-Управляющие-конструкции-SQL)
    - [Case](#user-content-Case)
    - [If](#user-content-If)
- [Основные объекты структуры базы данных](#user-content-Основные-объекты-структуры-базы-данных)
    - [Таблицы](#user-content-Таблицы)
    - [Представления](#user-content-Представления)
    - [Хранимые процедуры](#user-content-Хранимые-процедуры)
    - [Триггеры](#user-content-Триггеры)
    - [Функции](#user-content-Функции)
    - [Индексы](#user-content-Индексы)
    - [Пользовательские типы данных](#user-content-Пользовательские-типы-данных)
    - [Ограничения целостности](#user-content-Ограничения-целостности)
    - [Правила](#user-content-Правила)
    - [Умолчания](#user-content-Умолчания)
- [Таблица](#user-content-Таблица)
    - [Создание таблицы](#user-content-Создание-таблицы)
    - [Создание на основе запроса (Oracle)](#user-content-Создание-на-основе-запроса-Oracle)
    - [Изменение таблицы](#user-content-Изменение-таблицы)
    - [Удаление таблицы](#user-content-Удаление-таблицы)
- [Индексы](#user-content-Индексы)
- [Запрос получения данных `SELECT`](#user-content-Запрос-получения-данных-SELECT)
    - [`WHERE`](#user-content-WHERE)
        - [Сравнение блоков данных](#user-content-Сравнение-блоков-данных)
    - [ORDER BY](#user-content-ORDER-BY)
    - [Соединения и операции над отношениями](#user-content-Соединения-и-операции-над-отношениями)
        - [Построение вычисляемых полей](#user-content-Построение-вычисляемых-полей)
    - [GROUP BY](#user-content-GROUP-BY)
    - [HAVING](#user-content-HAVING)
    - [Подзапрос](#user-content-Подзапрос)
    - [Конструктор блока данных](#user-content-Конструктор-блока-данных)
    - [WITH](#user-content-WITH)
    - [Аналитический SQL](#user-content-Аналитический-SQL)
- [Запросы модификации данных](#user-content-Запросы-модификации-данных)
    - [Запрос добавления INSERT](#user-content-Запрос-добавления-INSERT)
        - [Первая форма оператора INSERT с параметром VALUES](#user-content-Первая-форма-оператора-INSERT-с-параметром-VALUES)
        - [Вторая форма оператора INSERT с параметром `SELECT`](#user-content-Вторая-форма-оператора-INSERT-с-параметром-SELECT)
        - [Третья форма оператора INSERT в виде UPDATE в MySQL](#user-content-Третья-форма-оператора-INSERT-в-виде-UPDATE-в-MySQL)
    - [Запрос удаления DELETE](#user-content-Запрос-удаления-DELETE)
    - [Запрос очистки таблицы TRUNCATE](#user-content-Запрос-очистки-таблицы-TRUNCATE)
    - [Запрос обновления UPDATE](#user-content-Запрос-обновления-UPDATE)
    - [Запрос вставки или обновления (MySQL)](#user-content-Запрос-вставки-или-обновления-MySQL)
    - [MERGE (Oracle)](#user-content-MERGE-Oracle)
- [Целостность данных](#user-content-Целостность-данных)
    - [Аномалии операций с данными](#user-content-Аномалии-операций-с-данными)
    - [Нормализация](#user-content-Нормализация)
    - [Вторая нормальная форма](#user-content-Вторая-нормальная-форма)
    - [Третья нормальная форма](#user-content-Третья-нормальная-форма)
- [Представления](#user-content-Представления)
- [Встроенные функции](#user-content-Встроенные-функции)
    - [Математические функции](#user-content-Математические-функции)
    - [Строковые функции](#user-content-Строковые-функции)
    - [Функции для работы с датой и временем](#user-content-Функции-для-работы-с-датой-и-временем)
- [Хранимые процедуры](#user-content-Хранимые-процедуры)
- [Триггеры](#user-content-Триггеры)
- [Транзакции](#user-content-Транзакции)
    - [Блокировки](#user-content-Блокировки)
    - [Управление транзакциями](#user-content-Управление-транзакциями)
- [Управление доступом к данным](#user-content-Управление-доступом-к-данным)
    - [Оператор GRANT](#user-content-Оператор-GRANT)
    - [Оператор REVOKE](#user-content-Оператор-REVOKE)
...
**Искусственный ключ, суррогатный ключ** - ключ, построенный на атрибуте, искусственно добавленном в отношение с единственной целью - гарантированно идентифицировать кортежи отношения.
...
**Атрибут** - именованное свойство сущности (отношения) - столбец.
...
CASE <значение>
    ...
    [WHEN [<сравниваемое_значение_2>] THEN <результат> ...]
    [ELSE <результат>]
END
```
...
## <a id="Пользовательские-типы-данных" href="#Пользовательские-типы-данных">Пользовательские типы данных</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
Кластерные индексы можно считать классическим случаем неплотных индексов, а некластерные - плотных.
...
```sql
<имя_поля> IN (<значение_1> [,...n])
```

```sql
(<имя_поля_1> [,...n]) IN ( (<значение_1> [,...n]) [,...m])
```
...
## <a id="WITH" href="#WITH">WITH</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
## <a id="Аналитический-SQL" href="#Аналитический-SQL">Аналитический SQL</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
`SELECT <поле_1>, <аналитический_запрос_1> AS <алиас> FROM <таблица>`
...
## <a id="MERGE-Oracle" href="#MERGE-Oracle">MERGE (Oracle)</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
USING <выборка> [<алиас_выборки>]
    ON <условия>
WHEN MATCHED THEN
    UPDATE SET <имя_таблицы>.<колонка_1> = <значение> [,...n] [WHERE <условия_включения_в_обновление>]
    [DELETE WHERE <условия_удаления_из_таблицы_по_обновленным_строкам>]
WHEN NOT MATCHED THEN
    INSERT (<колонка_1> [,...n]) VALUES (<значение> [,...n])
...
## <a id="Аномалии-операций-с-данными" href="#Аномалии-операций-с-данными">Аномалии операций с данными</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
## <a id="Нормализация" href="#Нормализация">Нормализация</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

## <a id="Вторая-нормальная-форма" href="#Вторая-нормальная-форма">Вторая нормальная форма</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
## <a id="Третья-нормальная-форма" href="#Третья-нормальная-форма">Третья нормальная форма</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
**Нетривиальная функциональная зависимость** - функциональная зависимость, которая может быть нарушена, что выполняется при следующем условии:` X -> Y, X ⊅ Y`.
...
Вызываются как: `<имя_функции> ([<аргумент_1> [,...n]])`
...
`<строка_1>||<строка_2>[||...]` - (oracle) конкатинирует строки (соединяет).  
...
<a id="Управление-доступом-к-данным" href="#Управление-доступом-к-данным">Управление доступом к данным</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
============================
...
## <a id="Оператор-GRANT" href="#Оператор-GRANT">Оператор GRANT</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
## <a id="Оператор-REVOKE" href="#Оператор-REVOKE">Оператор REVOKE</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
Генерация GUID в формате `XXXXXXXXXXXX-XXXXXXXXXXXX-XXXXXXXX`:
```sql
select substring(upper(hex(randomblob(16))), 0, 12)
    || '-'
    || substring(upper(hex(randomblob(16))), 0, 12)
    || '-'
    || substring(upper(hex(randomblob(16))), 0, 8)
    as uuid
```