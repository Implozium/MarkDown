...
- [Основы скриптов bash](#user-content-Основы-скриптов-bash)
    ...
    - [Планирование запуска скриптов](#user-content-Планирование-запуска-скриптов)
    - [Запуск скриптов по расписанию](#user-content-Запуск-скриптов-по-расписанию)
    - [`flock`](#user-content-flock)
...
- [Структура Linux](#user-content-Структура-Linux)
    ...
    - [Загрузка ядра](#user-content-Загрузка-ядра)
        - [Команда `systemd`](#user-content-Команда-systemd)
            - [journalctl](#user-content-journalctl)
            - [systemctl](#user-content-systemctl)
            - [Юниты](#user-content-Юниты)
...
## <a id="flock" href="#flock">`flock`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

`flock -n /run/lock/<имя>.lock -c 'команда'` - выполняет команду только если блокировки `/run/lock/<имя>.lock` нет и ставит блокировку на все время выполения команды.
...
- `/proc` - содержит информацию о запущенных в данный момент процессах, а также о некоторых параметрах ядра в виде "каталог-файл";
    - `/proc/<pid>/` - содержит описание процесса:
        - `self` - содержит описание текущего процесса;
        - `comm` - имя исполняемого файла;
        - `cmdline` - содержит полную команду, с помощью которой был запущен процесс, включая все аргументы;
        - `io` - содержит статистику ввода-вывода;
        - `fd/<id>` - содержит файловый дескриптор;
        - `fd/0` - содержит файловый дескриптор `stdin`;
        - `fd/1` - содержит файловый дескриптор `stdout`;
        - `fd/2` - содержит файловый дескриптор `stderr`;

        Ко всем файлам можно обратиться на чтение через `cat`:
        > `cat /proc/1/fd/1`
...
`mkfs -t <файловая_система> [-L <имя_раздела>] <раздел>` - создает файловую систему на указаном разделе.
...
`fsck [-p] [-n] <устройство>` - проверяет файловую систему в режиме интерактивного руководства [`-p` с автоматическим исправлением типичных проблем] [`-n` только проверка без изменений].
...
#### <a id="journalctl" href="#journalctl">journalctl</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
#### <a id="systemctl" href="#systemctl">systemctl</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
#### <a id="Юниты" href="#Юниты">Юниты</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
Секция `[Unit]` - сообщает некоторые подробности о юните и содержит описание и сведения о зависимости. Основные ключи:
...
- `-T <файл>` - указывает что с файла нужно загружать список файлов для архивирования.
- `-f <файл>` - "file" - использовать имя файла архива `<файл>` для распаковки указаное после ключей.
...
`tar [<ключи>] [<название_архива>] [<имя_файла1>[ ...]]`

Значения ключей:
- `-r` - добавить файлы;
- `-u` - обновляет файлы в архиве, только если они новее, чем в архиве, если они есть, а иначе добавляет их;
- `-f <файл>` - "file" - использовать имя файла архива `<файл>` указаное после ключей.
...
Ключи:
- `-f` - переводит процесс ssh в фоновый режим в текущем терминале;
- `-n` - предотвращает чтение из STDIN;
- `-N` - указывает, что не нужно выполнять удаленные команды;
- `-C` - устанавливает сжатие данных;
- `-T` - отменяет переназначение терминала;
- `-o StrictHostKeyChecking=no` - игнорирует смену публичного ключа сервера.

`ssh <user>@<server> [-t] <команда>[ <аргумент1>[ ...]]` - выполняет команду на удаленном сервере и тут же закрывает соединение [с управляющим терминалом для команд]. Для таких команд можно использовать pipe `|`, stdin `<`, stout `>` как для простых программ, при этом они работают после выполнения команды на сервере.

> `ssh user@8.8.8.8 command > my_file`  
> `mycommand | ssh user@8.8.8.8 "scp - user@10.1.1.2:/path/to/file"` - позволяет прокидывать вывод команды с клиента через сервер на сервер.  
> `ssh example.com -t "cd /www/example.com && git status -sb | head -1"`  
> `tar -c * | ssh user@server "cd && tar -x"` - запаковывает файлы локально и пишет результат в stdout, откуда их читает ssh, передает в stdin на удаленном сервере, где их cd игнорирует (не читает stdin), а tar - читает и распаковывает
...
Настройки соединения по умолчанию находятся в конструкции `Host *`.

`/etc/ssh/sshd_config` - общий файл с конфигурацией. Может содержать:
- `PermitRootLogin no` - закрывает доступ по паролю от root-пользователя;
- `AllowTcpForwarding yes` - включает переадресацию TCP;
- `GatewayPorts yes` - включает переадресацию портов на интерфейсы;
- `ClientAliveInterval <интервал>` - включает на сервере heartbeat-сообщения раз в `<интервал>`;
- `ClientAliveCountMax <количество>` - устанавливает на сервере максимальное количество `<количество>` heartbeat-сообщений;
- `ServerAliveInterval <интервал>` - включает на клиенте heartbeat-сообщения раз в `<интервал>`;
- `ServerAliveCountMax <количество>` - устанавливает на клиенте максимальное количество `<количество>` heartbeat-сообщений;
- `Compression yes` - включает сжатие.

`ssh -L [<локальный_адрес>:]<локальный_порт>:<удаленный_адрес>:<удаленный_порт> [<user>@]<server>` - перенаправляет все локальные подключения к `<локальный_адрес>:<локальный_порт>` на `<удаленный_порт>` порт сервера `<удаленный_адрес>` через SSH-сервер `<server>`. Трафик между локальной системой и SSH-сервером идет по SSH-туннелю. Трафик между SSH-сервером и `<удаленный_адрес>` - нет. С точки зрения `<удаленный_адрес>` трафик идет от SSH-сервера. По умолчанию `<локальный_адрес>` соответствует `127.0.0.1`. Возможно использование нескольких ключей `-L` в одном клиенте.

> `ssh -L 127.0.0.1:8080:example.org:80 ssh-server`

`ssh -R [<удаленный_адрес>:]<удаленный_порт>:<локальный_адрес>:<локальный_порт> [<user>@]<server>` - перенаправляет трафик с порта `<удаленный_порт>` SSH-сервера `<server>` для всех интерфейсов на `<локальный_порт>` порт на `<локальный_адрес>`. Если один из интерфейсов смотрит в Интернет, трафик, адресуемый на порт `<удаленный_порт>`, будет перенаправляться на `<локальный_адрес>`. Если указан `<удаленный_адрес>`, то доступ ко входу в SSH-туннель со стороны сервера разрешен только с этого адреса. Трафик между локальной системой и SSH-сервером идет по SSH-туннелю. Трафик между SSH-сервером и `<локальный_адрес>` - нет. С точки зрения `<локальный_адрес>` трафик идет от локальной системы.

> `ssh -R 8080:localhost:80 ssh-server`

`ssh -D [<локальный_адрес>:]<локальный_порт> [<user>@]<server>` - поднимает SOCKS-прокси на `<локальный_порт>` порту на `<локальный_адрес>` для всех интерфейсов в локальной системе. Теперь трафик, отправленный через прокси-сервер на SSH-сервер, можно адресовать на любой порт или конечный хост. По умолчанию используется протокол SOCKS5, поддерживающий TCP и UDP. С точки зрения конечных хостов трафик идет от SSH-сервера.
...
`ssh -J <jump_адрес> <целевой_адрес>` - *SSH‑over‑SSH*, указывает что нужно подключиться к `<jump_адрес>`, который должен установить SSH‑соединение с `<целевой_адрес>` при этом использую `<jump_адрес>` в виде прокси.
...
`scp -r <user>@<server>:<путь_к_каталогу_на_сервере> <путь_к_локальному_каталогу>` - осуществляет копирование каталога через ssh-сессию с сервера.
...
- `rm <путь>` - удаляет файл по пути с сервера.
...
`screen -r` - возвращает к заранее свернутому скрину.
...
`screen -r <имя>` - возвращает к заранее свернутому именному скрину.

`screen -list` / `screen -ls` - выводит список запущенных скринов.

<a id="tmux" href="#tmux">`tmux`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
=====

`tmux` - создание сессии.
`tmux new-session -s <имя>` - создание сессии с определенным именем.
`tmux ls` - выводит список сессий с их именами.
`tmux attach -t <имя>` - подключается к сессии.

Комбинации клавиш:
- `Ctrl+b d` - отключается от сессии;
- `Ctrl+b "` - создание горизонтальной панели;
- `Ctrl+b %` - создание вертикальной панели;
- `Ctrl+b z` - раскрывает панель на весь экран, а если она раскрыта, то возвращает предыдущий вид панелей;
- `Ctrl+b c` - создает окно;
- `Ctrl+b &` - закрывает окно;
- `Ctrl+b f` - показывает список окон;
- `Ctrl+b x` - закрывает активную панель;
- `Ctrl+b o` - переходит на следующую панель;
- `Ctrl+b Ctrl+o` - вращает панели;
- `Ctrl+b <стрелка>` - делает активной панель следующей за стрелкой;
- `Ctrl+b Ctrl+<стрелка>` - увеличивает или уменьшает размер активной панели.
...
- `-path <путь>` - указывает по какому пути искать в каталоге `<каталог>`; поддерживаются wildcards (шаблоны замены).
...
`id [<имя_пользователя>]` - выводит информацию о текущем [указанном `<имя_пользователя>`] пользователе, его идентификатор **uid**, идентификатор основной группы **gid** и других группах в которых он состоит.
...
`w [<имя_пользователя>]` - выводит список подключенных пользователей [указаного `<имя_пользователя>`] и их терминалов.
...
`last [<имя_пользователя>]` - выводит информацию о последних входах в систему и перезагрузках текущего [указаного `<имя_пользователя>`] пользователя.
...
`lsof -i -P -n | grep LISTEN` - показывает какие приложения слушают порты.

## <a id="ip" href="#ip">`ip`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
`ip [-6] [-4] addr list` - отображает список сетевых адресов [только ipv6] [только ipv4].

`ip [-6] [-4] addr add <ip> dev <интерфейс>` - добавляет ip [ipv6] [ipv4] на интерфейс.  
> `ip -6 addr add 2a02:69e0:1337::1/64 dev eth0`

`ip [-6] [-4] addr del <ip> dev <интерфейс>`- удаляет ip [ipv6] [ipv4] из интерфейса.

## <a id="iptables" href="#iptables">`iptables`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

Используется для фильтрации пакетов по цепочке правил. Пакеты делятся на виды:
- `input` - входящие пакеты и подключения;
- `forward` - проходящие пакеты и подключения, которые не предназначены для целевой машины;
- `output` - выходящие пакеты и подключения.
...
- `nat` - обеспечивает работу nat, при работе в качестве маршрутизатора;
- `filter` - основная таблица для фильтрации пакетов, используется по умолчанию.

`iptables [-t <таблица>] <действие> [<цепочка>]` - где `<действие>` это:
...
- `--dport <порт>` - определенный порт;
- `-j` - выбрать действие, если правило подошло.
> `sudo iptables -A INPUT -p tcp --dport 8001 -j ACCEPT` - открывает порт 8081

`iptables -L [<цепочка>]` - выводит список правил [для указаной цепочки].  
`iptables -F [<цепочка>]` - очищает правила [для указаной цепочки].

`iptables -P <цепочка> <действие_по_умолчанию>` - устанавливает действие по умолчанию для цепочки.

`iptables -A <цепочка> -s <ip> -j <действие>` - добавляет правило для цепочки.
> `iptables -A INPUT -s 10.10.10.10 -j DROP`

`iptables -D <цепочка> -s <ip> -j <действие>` - удаляет правило для цепочки.

Для протокола v6 есть `ip6tables`.

## <a id="Общее" href="#Общее">Общее</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
`ss -tupln` - показывает информацию о текущих сетевых соединениях, активных сетевых процессах, сокетах с адресами и имена процессов с текущими TCP/UDP-соединениями.

`nc -z <домен> <порт>` - проверяет открытый порт на открытость. Флаги:
- `-w <количество_сек>` - устанавливает ожидание равное `<количество_сек>`;
- `-u` - использует протокол UDP;
- `-v` - выводит информацию.

`nc -l <порт>` - начинает слушать порт.  
`nc -lvp <порт>` - аналогично, но выводит информацию о данных пришедших на порт.

`echo 1 > /proc/sys/net/ipv4/ip_forward` - вкючает пересылку пакетов.
...
`/etc/resolv.conf` - содержит конфигурацию DNS.

`host <домен>` - возвращает ip-адрес указаного домена и все его dns-алиасы.

`nslookup [<опции>] <домен>` - проверяет разрешение DNS-имени. Ответ бывает от: **authoritative answer** (авторитетный ответ) - это ответ, который получен от основного (официального) сервера и **non-authoritative answer** (неавторитетный ответ) - это ответ от промежуточного сервера. Опции:
- `-type=MX` - для определения MX-записи - электронной почты;
- `-type=NS` - для определения NS-записи - авторитетного сервера;
- `-type=SOA` - для определения SOA-записи - эталонной записи о домене.

`hostname` - выводит текущее имя хоста.  
`hostname <новое_имя>` - задает новое имя хоста.

`/etc/hosts` - хранятся соответствия между IP-адресом и доменным именем. Имеет структуру в формате строк: `<IP_адрес> <DNS_имя> [<алиас_1>[ ...]]`.

`sudo killall -HUP mDNSResponde` - перезагружает службу DNS.
...
`apt info <имя_пакета>` - аналогично.  
...
`service --status-all` - выводит список сервисов.  
`service <сервис> start` - запускает сервис.  
`service <сервис> stop` - останавливает сервис.  
`service <сервис> restart` - перезапускает сервис.  
`service <сервис> reload` - перезагружает конфигурацию сервиса.
...
`cp <откуда>[ ...] <куда>` - копирует файлы, если `<куда>` - существующая директория, то копирует файлы в нее, если `<откуда>` директория и путь оканчивается `/`, то будет скопировано только все содержимое, а иначе вся директория. Дополнительные параметры:
...
`ln <файл> <имя_ссылки>` - создает жесткую ссылку на файл. Таким образом, получается что файл присутствует в системе под несколькими разными именами. Файл существует до тех пор, пока с его инодом связано хотя бы одно имя. Понятия "*жесткая ссылка на файл*" и "*имя файла*" являются синонимами.

`ln -s <объект> <имя_ссылки>` - создает символьную ссылку на любой объект (файл, директория, сокет...) любой файловой системы. *Симлинк* это специальный вид файла, который ссылается на другой файл по имени, а не напрямую на инод. Симлинки не предохраняют файл от удаления.
...
`tailf <файл>` - просмотр записи в текстовый файл в реальном времени.  
`tail -f <файл>` - аналогично.
...
`tee <файл>` - читает данные из стандартного ввода и пишет их одновременно в файл и в стандартный поток вывода. Основные флаги:
- `-a` - дописывает в файл и добавляет после пустую строку.
...
`date -Iseconds` - выводит текущую дату в консоль в формате ISO с секундами.  
...
`exec bash` - перезагружает терминал.

`base64 [-d] [-w <количество_символов_в_строке>] [<файл>]` - производит кодирование [декодирование] содержимого из стандартного потока ввода [файла] в формат base64 с переносом строк размером 76 [указанным `<количество_символов_в_строке>` при 0 - нет переносов].
...
`while :; do <команда>; done` - запускает команду в бесконечном цикле.

`export $(cat <файл>) > /dev/null && <программа>` - запускает программу `<программа>`, для которой будут установлены переменные окружения из файла `<файл>`.

`find . -name "*.<расширение>" -exec wc -l {} \; | awk '{ SUM += $0} END { print SUM }'` - считает количество строк во всех файлах.

`cat < /dev/tcp/<domain>/<port>` - позволяет считать информацию по порту указанного домена:
> `cat < /dev/tcp/time.nist.gov/13`

```bash
exec <дескриптор><>/dev/tcp/<domain>/<port> # перенаправляет ввод и вывод в дескриптор
echo -e "<текст>" >&<дескриптор> # пишет в дескриптор
cat <&<дескриптор> | head # выводит ответ из дескриптора
```
...
`nc -z 0 <port> || ssh -f -N -L <port>:<host>:<port_2> <user>@<server>` - запускает команду `ssh`, если порт `<port>` не прослушивается:
> `nc -z 0 12345 || ssh -f -N -L 12345:127.0.0.1:12345 user@server`

`find . -name "<имя_файла>" -type f -exec mv '{}' '{}.<суффикс>' \;` - добавляет к файлам суффикс.
> `find . -name "overridden.ts" -type f -exec mv '{}' '{}.old' \;`

`find . -name "<имя_файла>.<суффикс>" -type f -exec sh -c 'file="{}"; mv "$file" "${file%.<суффикс>}"' \;` - удаляет у файлов суффикс.
> `find . -name "overridden.ts.old" -type f -exec sh -c 'file="{}"; mv "$file" "${file%.old}"' \;`

Запускает программу `<программа>` с режимом перезапуска в случае ошибки и записью логов выполнения и ошибки в файлы:
```bash
while :
do
    file_prefix=$(date -Iseconds);
    <программа> > logs/$1/logs_${file_prefix}.out 2> logs/$1/errors_${file_prefix}.err < /dev/null &
    id=$!;
    echo "Process id ${id}";
    wait $id;
done
