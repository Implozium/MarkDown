...
- [`nginx`](#user-content-nginx)
    ...
    - [Глобальные переменные](#user-content-Глобальные-переменные)
...
- `docker [image] build [<опции>] <путь_к_директории_сборки>` - собирает образ Docker из `Dockerfile` из набора файлов, расположенных по определенному пути `<путь_к_директории_сборки>` или URL, с указанными опциями `<опции>`:
    ...
    - `-t <имя_образа>[:<тег>]` - устанавливает имя образа [и тег];
...
- `docker [container] run [<опции>] <образ> [[-it] <команда> [, <аргумент1_для_команды>[ ...]]]` - создает и запускает контейнер на основе указанного образа `<образ>`[, а в нем указаную команду] с указанными опциями `<опции>`:
    - `{-p | --publish} <порт>:<expose_порт>[/udp]` - с указанием проброса порта `<expose_порт>` из контейнера на порт `<порт>` хоста [по протоколу UDP];
...
- `docker [container] exec -it <контейнер> <команда>` - подключается к контейнеру и выполняет в нем команду `<команда>`;
    > `docker exec -it <контейнер> sh` - подключается к докеру для выполнения команд, где `exit` - выход из докера.

Для выполнения команд внутри команды в контейнера используется такая конструкция:
```bash
docker exec <контейнер> <команда> <<EOF
# <команды>
EOF
```
...
`docker run "$(docker build -q .)"` - собирает и запускает текущий образ.

`docker manifest inspect <image>` - выводит информацию о манифестах для указанного образа.
...
services:
    <имя_сервиса>:
        ...
        build: <путь>
        build:
            context: <путь_к_Dockerfile>
            # определяет значения ARG для сборки
            args:
                <имя_переменной>: <значение>
            # включает сеть хоста для контейнера
            network: host
        ...
        ports:
            - <порт_компьютера>:<порт_контейнера>[/udp]
        ...
        depends_on:
            ...
            <имя_сервиса>:
                ...
                condition: service_started # ожидание пока сервис не будет успешно запущен
                condition: service_completed_successfully # ожидание пока сервис не закончит успешное выполнение
...
Для команды `docker compose` есть вариант `docker compose -f <имя_файла>.yml` который будет применять указанный файл `<имя_файла>.yml` вместо `compose.yml`. Эту команду нужно использовать всегда, чтобы взаимодействовать с определенным файлом.
...
`docker compose exec [-T] <сервис> <команда>` - выполняет команду в выполняющемся контейнере [без использования псевдоконсоли].

Для выполнения команд внутри команды в контейнера используется такая конструкция:
```bash
docker compose exec -T <сервис> <команда> <<EOF
# <команды>
EOF
```
...
- **кластер** - состоит из одного или нескольких мастер-узлов и рабочих узлов:
    - **мастер-узлы** - управляют состоянием кластера, отвечают за планирование и координацию работы контейнеров:
        ...
        - **Scheduler** - определяет, на каких узлах будут запускаться новые **Pods** - набор контейнеров;
    ...
    - **рабочие узлы** (Nodes) - запускают контейнеры и управляют ими:
        ...
        - **пользовательские Pods** - это самая маленькая и базовая единица развёртывания в Kubernetes, где каждый **Pod** - это один или несколько контейнеров, они имеют общие сетевые ресурсы - делят один IP-адрес и могут общаться через `localhost` и хранилище;
...
**Объект Kubernetes** - это сущность, которая описывает состояние кластерных ресурсов: приложения, доступные им вычислительные ресурсы, политики поведения и безопасности, а также многое другое.  
Объект Kubernetes - это *record of intent*, или "запись о намерениях".
...
`kubectl cluster-info` - выводит информацию о кластере.
...
**Deployment** - поведение аналогично **ReplicaSet**, но с дополнительными возможностями по плавной выкатке подов. При создании создает **ReplicaSet**, который порождает нужное количество подов и берется следить, чтобы их количество не менялось. При необходимости перевыкатки возьмет на себя создание второго **ReplicaSet** и постепенное масштабирование их обоих до тех пор, пока в старом не останется подов.
...
`kubectl get deployment <имя_деплоя>` - выводит информацию о размещении.  
`kubectl describe deployment <имя_деплоя>` - выводит подробную информацию о размещении.
...
**HorizontalPodAutoscaler** (HPA) - это ресурс по автоматическому горизональному масштабированию. Мониторит определенные метрики (например, CPU или память) и автоматически регулирует количество подов в **Deployment**, **StatefulSet** или **ReplicaSet**, чтобы поддерживать целевые значения этих метрик.
...
spec:
  ...
  - from:
      - podSelector:
          matchLabels:
            <ключ_ingress_1>: <значение>
  egress:
  - to:
      - podSelector:
          matchLabels:
            <ключ_egress_1>: <значение>
```

Позволяет:
- взаимодействовать подам с метками: `<ключ_ingress_1>: <значение>` с подами с метками: `<ключ_1>: <значение>`;
- взаимодействовать подам с метками: `<ключ_1>: <значение>` с подами с метками: `<ключ_egress_1>: <значение>`.

## <a id="minikube" href="#minikube">minikube</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

`minikube start --driver=hyperkit [--addons=<аддон>]` - запускает эмуляцию кластера kubernetes с гипервизором для локальной разработки.
...
`minikube addons enable metrics-server` - включает поддержку metrics-server.
...
- `concat(<список_1>[, …])` - соединяет списки и возвращает новый список;
...
- `-F '<имя_поля>=<значение>'` - добавляет значение в указаное поле;
...
Использование флагов включает определенные типы заголовков содержимого:
...
## <a id="Глобальные-переменные" href="#Глобальные-переменные">Глобальные переменные</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
