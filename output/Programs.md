...
- `docker container run [<опции>] <образ> [-it <команда>]` - запускает контейнер на основе указанного образа `<образ>`[, а в нем указаную команду] с указанными опциями `<опции>`:
...
- `docker run ...` - аналогично предыдущей;
