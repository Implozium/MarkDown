...
`const <Обернутый_компонент> = connect(<функция_соотвествия_состояния> [, <функция_соотвествия_действий>])(<Компонент>)` - подключает React компонент к Redux `store`. Возвращает новый присоединенный компонент, который оборачивает переданный компонент - `<Connected(<Компонент>)>` и добавляет свойство, которое указывает на метод redux store - `dispatch`.

`<функция_соотвествия_состояния>` (`mapStateToProps`) - преобразует состояние к свойствам для компонента, т.е. соответствие `state` к `props`, которая должна выглядеть как:
```javascript
function <функция_соотвествия_состояния>(state) {
    ...
    return {<свойство>: <значение> [, ...]};
}
```

`<функция_соотвествия_действий>` (`mapDispatchToProps`) - преобразует действия к свойствам для компонента, т.е. соответствие `actions` к `props`, которая должна выглядеть как:
```javascript
function <функция_соотвествия_состояния>(dispatch) {
    ...
    return {
        <dispatchedAction>: () => {...}
    };
