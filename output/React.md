...
class <Компонент> extends React.Component {
    ...
    // этод метод не вызывается в момент первого render'a. В этот момент, старые props доступны как this.props, а новые props доступны в виде nextProps. Если вызывать setState внутри этого метода - не будет вызван дополнительный render
    // [DEPRECATED]
    ...
    // вызывается сразу после render, когда именение еще не принято в DOM, позволяет захватить некотурую информацию из DOM
...
const <Обернутый_компонент> = connect(
    [<функция_соотвествия_состояния>, ]
    [<функция_соотвествия_действий>, ]
    [<функция_слияния>, ]
    [<опции>]
)(<Компонент>)
```
...
##### <a id="Эфекты" href="#Эфекты">Эфекты</a>
...
function* <action_saga>() {
    yield takeEvery('<action>', <saga>); // производит связывание саги-генератора с действием, которая вызовится по определенному действию
...
function* <root_saga>() {
    yield all([ // запускает паралельно саги на выполнение
        <saga>()
        [, ...]
    ]);
}
