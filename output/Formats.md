...
- [Протоколы](#Протоколы)
    ...
    - [REST](#REST)
        ...
        - [JSON:API](#JSONAPI)
            ...
            - [Получение ресурса](#Получение-ресурса)
...
- `related` - строка с относительной ссылкой на текущий ресурс;
...
- для запроса ресурса: `null`, если ресурса нет или объектом ресурса;
- для запроса множества ресурсов: массив объектов ресурса или пустой массив.
...
- `relationships` - объект с отношениями ресурса к другим ресурсам - связи, представляет из себя объект где ключи - это имя связи, а значения - объекты:
    - `links` - объект с ссылками на связь, где `self` должно иметь вид `<тип_ресурса>/<идентификатор_ресурса>/relationships/<имя_связи>`;
    - `data` - данные связанного ресурса:
        ...
        - для связи один ко многим может быть массивом объектов ресурса, в основном состоящие из полей `type` и `id` или пустым объектом.
- `links` - объект с ссылками на ресурс;
...
- `detail` - строковой человеко читаемое описание ошибки.
...
`include=<тип_дополнительного_ресурса_1>[.<тип_связанного_ресурса_с_ресурсом_1>[....]][,...]` - указывает, что вместе с ресурсом загрузить еще его связи: `<тип_дополнительного_ресурса_1>` [и ресурс `<тип_связанного_ресурса_с_ресурсом_1>`] и поместить в поле `included` в ответе:
...
`sort=[-]<только_поле_1>[,...]` - указывает по каким полям производить сортировку `-` перед полем указывает на обратную сортировку по полю:
> `/articles?sort=-created,title`

`filter` - используется для фильтрации ответа.

Для фильтрации используется `page[offset]` - отступ и `page[limit]` - количество:
...
Для получения связи одного ресурса отправляется `GET` запрос на `/<тип_ресурса>/<идентификатор>/relationships/<имя_связи>` или `/<тип_ресурса>/<идентификатор>/<имя_связи>`.

В ответе сервера при успешном получении ресурса содержится:
- код 200;
- JSON-объект с ресурсом.

При неуспешном поиске должен быть код 404 (для связей если родительского ресурса не существует) и ошибку или код 200 и `data` = `null`.
...
В ответе сервера при успешном создании ресурса содержится:
- код 201, 202, 204;
- JSON-объект с ресурсом при коде 201.
...
- `data` - объект с данными для обновления, при этом связи ресурса со внешними ресурсами должны передаваться в поле `relationships` и все что содержится в полях заменяет связи на новые;

В ответе сервера при успешном обновлении ресурса содержится:
- код 200, 202, 204;
- JSON-объект с ресурсом при коде 200.
...
Связи можно обновлять `PATCH` запрос на `<тип_ресурса>/<идентификатор_ресурса>/relationships/<имя_связи>` с JSON-объектом:
- `data` - объект с данными для обновления связей и все что содержится в полях заменяет связи на новые;
...
В ответе сервера при успешном удалении ресурса содержится:
- код 200, 202, 204.