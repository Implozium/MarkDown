...
```
Символы:
- `>` - в этом виде строки соединяются пробелами;
- `|` - в этом виде строки соединяются переносами строк.
...
## <a id="Словари" href="#Словари">Словари</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
`include=<имя_связи_ресурса_1>[.<имя_связи_ресурса_связанного_с_ресурсом_1>[....]][,...]` - указывает, что вместе с ресурсом загрузить еще его связи: `<имя_связи_ресурса_1>` (имя связи всегда должно идти от главного ресурса, даже при запросе его свзяей) [и ресурс `<имя_связи_ресурса_связанного_с_ресурсом_1>`] и поместить в поле `included` в ответе:
> `/articles/1?include=comments.author,ratings`  
> `/articles/1/relationships/comments?include=comments.author`
...
- `data` - объект с идентификаторами ресурсов для удаления связей, если связи такой нет, то ничего не будет происходить;
