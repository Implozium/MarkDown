...
- [Основы Git](#Основы-Git)
    ...
    - [Просмотр истории коммитов](#Просмотр-истории-коммитов)
        ...
        - [Общий лог `git shortlog`](#Общий-лог-git-shortlog)
        - [Лог команд `git reflog`](#Лог-команд-git-reflog)
        - [Аннотация файла `git blame`](#Аннотация-файла-git-blame)
...
`git clone [--bare] <url> [<имя_результирующего_каталога>]` - создаёт каталог с именем окончания **url**, инициализирует в нём каталог `.git` [только каталог `.git`], скачивает все данные для этого репозитория и создаёт (**checks out**) рабочую копию последней версии.
...
`git add <новый_файл>` - начинает отслеживать (добавить под версионный контроль) новый файл, также принимает параметром путь к файлу или каталогу, если это каталог, команда рекурсивно добавляет (индексирует) все файлы в данном каталоге.  
...
### <a id="Команда-log" href="#Команда-log">Команда `log`</a>
...
#### <a id="Дополнительное-ограничения-вывода" href="#Дополнительное-ограничения-вывода">Дополнительное ограничения вывода</a>
...
### <a id="Общий-лог-git-shortlog" href="#Общий-лог-git-shortlog">Общий лог `git shortlog`</a>
...
### <a id="Лог-команд-git-reflog" href="#Лог-команд-git-reflog">Лог команд `git reflog`</a>

`git reflog [show]` - выводит список команд которые были применены [, применяет все параметры как и у команды `git log` если возможно].

### <a id="Аннотация-файла-git-blame" href="#Аннотация-файла-git-blame">Аннотация файла `git blame`</a>
...
`git clone [file://]/<путь_к_проекту>.git` - клонирует репозиторий.  
