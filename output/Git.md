...
`git clone <url> [<имя_результирующего_каталога>]` - создаёт каталог с именем окончания **url**, инициализирует в нём каталог `.git`, скачивает все данные для этого репозитория и создаёт (**checks out**) рабочую копию последней версии.
