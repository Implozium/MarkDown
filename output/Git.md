...
В файле `.gitconfig` хранятся настройки проекта. В виде:
```
[<секция>]
    <ключ> = <значение>
```
...
В файле `.gitconfig` указывается секция с хуками, где `<каталог>` - путь к каталогу с хуками.
```
[core]
    hooksPath = <каталог>
```
...
`!` - обозначает, что содержатся изменения которые нарушают обратную совместимость.
...
До тех пор, пока не будет связи с сервером **origin**, указатель **origin/master** не будет сдвигаться. Ветка master на сервером origin может в это время измениться.

`git remote add <имя_удал_репоз> {https://|git@}<путь_к_проекту>.git`

`git remote set-url <имя_удал_репоз> {https://|git@}<путь_к_проекту>.git` - устанавливает url у `<имя_удал_репоз>`.
