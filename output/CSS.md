...
- [Основные свойства](#user-content-Основные-свойства)
    ...
    - [Работа с потоком элементов](#user-content-Работа-с-потоком-элементов)
        ...
        - [Видимость элемента](#user-content-Видимость-элемента)
            - [Свойство `visibility`](#user-content-Свойство-visibility)
            - [Свойство `content-visibility`](#user-content-Свойство-content-visibility)
...
- [At-правила](#user-content-At-правила)
    - [At-правило `@media`](#user-content-At-правило-media)
    - [At-правило `@supports`](#user-content-At-правило-supports)
    - [At-правило `@container`](#user-content-At-правило-container)
    - [At-правило `@scope`](#user-content-At-правило-scope)
    - [At-правило `@layer`](#user-content-At-правило-layer)
...
`:has-slotted`                 | применяется, если в элементе `slot` есть элементы
...
`:state(<состояние>)`          | позволяет применить стиль к shadow хосту при условии что у shadow хоста есть состояние `<состояние>`
...
- `svw` - определяет ширину браузера с учетом показа всех динамических элементов браузера; 
- `svh` - определяет высоту браузера с учетом показа всех динамических элементов браузера;
- `lvw` - определяет ширину браузера без учета показа всех динамических элементов браузера;
- `lvh` - определяет высоту браузера без учета показа всех динамических элементов браузера;
- `dvw` - определяет динамическую ширину браузера в зависимости от показа динамических элементов браузера;
- `dvh` - определяет динамическую ширину браузера в зависимости от показа динамических элементов браузера;
...
Для фиксированой ширины используется такая конструкция `minmax(0, 1fr)` так как размер контейнера для `fr` зависит от его содержимого и рассчитывается от этого, когда имеет размер `auto` и поэтому они будут не фиксированного размера.
...
#### <a id="abs" href="#abs">`abs`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

`abs(<выражение>)` - это функция CSS, которая возвращает абсолютное значение выражения.

#### <a id="round" href="#round">`round`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

`round([<тип_округления>,] <значение>, <интервал_округления>)` - округляет значение `<значение>`, чтобы оно делилось без остатка на `<интервал_округления>` со стратегией округления `<тип_округления>`, которая принимает значения:
- `up` - округляет вверх;
- `down` - округляет вниз;
- `nearest` - (по умолчанию) округляет до ближайшего целого;
- `to-zero` - округляет по направлению к 0.

> `round(up, 101px, var(--interval));`

#### <a id="if" href="#if">`if`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

Используется в качестве значений свойств: `if(<условие_1>: <значение>[; ...]; else: <значение_по_умолчанию>);`, где условие может быть:
- `style(<имя_переменной>[: <значение_переменной>])` - истинно, когда `<имя_переменной>` установлено для текущего элемента [и равно значению `<значение_переменной>`];
- `media(<медиа_запрос>)` - истинно, когда медиа запрос `<медиа_запрос>` удовлетворен;
- `supports(<свойство>[: <значение>])` - истинно, когда свойство `<свойство>` поддерживается [со значением `<значение>`].

Условия могут содержать логические операторы `and` / `or` / `not` / `,` / `only`.

> `width: if(media(prefers-reduced-motion: no-preference) and style(--size: large): 300px; else: 100px);`
...
### <a id="Видимость-элемента" href="#Видимость-элемента">Видимость элемента</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

#### <a id="Свойство-visibility" href="#Свойство-visibility">Свойство `visibility`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
#### <a id="Свойство-content-visibility" href="#Свойство-content-visibility">Свойство `content-visibility`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

Отвечает за видимость содержимого. Принимает значения:
- `visible` - содержимое отображается;
- `hidden` - содержимое скрыто;
- `auto` - содержимое показывается в зависимости от видимости элемента. Работает вместе со свойством: `contain-intrinsic-size: <ширина> <высота>;`, которое отвечает за резервирование места под элемент.
...
<a id="At-правила" href="#At-правила">At-правила</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
==========

## <a id="At-правило-media" href="#At-правило-media">At-правило `@media`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
## <a id="At-правило-supports" href="#At-правило-supports">At-правило `@supports`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
## <a id="At-правило-container" href="#At-правило-container">At-правило `@container`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]
...
## <a id="At-правило-scope" href="#At-правило-scope">At-правило `@scope`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

Позволяет определить правила стилей которые будут действовать от `<селектор_от>` и ниже по DOM-дереву [и до `<селектор_до>`]:
```css
@scope (<селектор_от>) [to (<селектор_до>)] {
    <описание_CSS_правил>
}
```

## <a id="At-правило-layer" href="#At-правило-layer">At-правило `@layer`</a> [<a id="Содержание" href="#Содержание">Содержание</a>]

Позволяет определить слои которые будут использоваться для приоритета применения правил стилей к селекторам:
```css
@layer <имя_слоя> {
    <описание_CSS_правил>
}
...
Слои можно вкладывать друг в друга:
```css
@layer <имя_слоя> {
    <описание_CSS_правил>
    @layer <имя_вложенного_слоя> {
        <описание_CSS_правил>
    }
}
```

Правила для слоев можно дополнять посредством объявления слоя с таким же именем: `@layer <имя_слоя>[.<имя_вложенного_слоя>[. ...] {}`.

Приоритет указывается посредством определения: `@layer <имя_слоя_1>[, <имя_слоя_2> [, ...]]`. Если объявления найдены в нескольких слоях, то наивысшим приоритетом обладает последний слой. Правила без слоев имеют приоритет меньше всех слоев. Если эта конструкция не задана, то приоритет идет в порядке определения слоев.

`@import <url> layer(<имя_слоя>);` - помещает содержимое из `<url>` в слой `<имя_слоя>`.
